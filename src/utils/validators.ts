...
export function validateEmail(input: string): ValidationResult {
  ...
  const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!re.test(trimmed))
    return { valid: false, message: "Niepoprawny format email" };
  return OK;
}
...
export function validatePassword(input: string): ValidationResult {
  ...
  if (!/[A-Z]/.test(input))
    return { valid: false, message: "Wymagana wielka litera" };
...
export function validateVolume(value: unknown): ValidationResult {
  ...
  if (Number.isNaN(n))
    return { valid: false, message: "Wolumen musi być liczbą" };
  if (n <= 0) return { valid: false, message: "Wolumen musi być dodatni" };
  if (n > 1000)
    return { valid: false, message: "Wolumen nie może przekraczać 1000" };
  const rounded = Math.round(n * 100) / 100;
  if (rounded !== n)
    return { valid: false, message: "Dozwolone są max 2 miejsca po przecinku" };
  return OK;
}
...
export function validateSymbol(symbol: string): ValidationResult {
  ...
  if (!/^[A-Z]{3,10}(\.[A-Z]{2,4})?$/.test(symbol)) {
    return {
      valid: false,
      message: "Niepoprawny format symbolu (np. EURUSD, AAPL.US)",
    };
  }
  return OK;
}
...
export function validateStopLoss(
  ...
  if (stopLoss <= 0)
    return { valid: false, message: "Stop-loss musi być dodatni" };
  if (side === "BUY" && stopLoss >= openPrice) {
    return {
      valid: false,
      message: "Stop-loss dla BUY musi być niżej niż cena otwarcia",
    };
  }
  if (side === "SELL" && stopLoss <= openPrice) {
    return {
      valid: false,
      message: "Stop-loss dla SELL musi być wyżej niż cena otwarcia",
    };
  }
  return OK;
}
